"""
Given an unsorted array `Arr` with `n` positive integers, find the $k^{th}$ smallest element 
in the given array, using the Fast Select algorithm (a Divide & Conquer approach).
"""
...
def findMedian(Arr, start, size):
    ...
    return target_list[size // 2]


Arr = [6, 80, 36, 8, 23, 7, 10, 12, 42]
k = 5
result = fastSelect(Arr, k) # Outputs 12
print("Pass" if result == 12 else "Fail")

Arr = [5, 2, 20, 17, 11, 13, 8, 9, 11]
k = 5
result = fastSelect(Arr, k) # Outputs 11
print("Pass" if result == 11 else "Fail")

Arr = [6, 80, 36, 8, 23, 7, 10, 12, 42, 99]
k = 10
result = fastSelect(Arr, k) # Outputs 99
print("Pass" if result == 99 else "Fail")
